...
public abstract class AbstractGridPointTranslator implements GridPointTranslator {
	 ...
	 * @throws repast.space.SpatialException if the transform is invalid. For example, if the new location
	...
	public void transform(int[] transformedLocation, int... targetLocation) throws SpatialException {
		...
		System.arraycopy(targetLocation, 0, transformedLocation, 0, targetLocation.length);
	}

	protected void boundsCheck(int i, int value) {
		if (value < 0 || value >= dimensions.getDimension(i)) {
