...
import simbigraph.collections.Contains;
import simbigraph.collections.FilteredIterator;
import simbigraph.collections.IteratorOverIterables;
...
public class MooreQuery<T> extends AbstractGridQuery<T> {
	...
	private class Iterator2D extends IteratorOverIterables<T>  {
		...
		protected Iterable<T> getNext() {
			...
			if (x > xMax) return null;
			Iterable<T> tmp = grid.getObjectsAt(x, y);
			y++;
			return tmp;
		}
	...
	private class Iterator3D extends IteratorOverIterables<T> {
		...
		protected Iterable<T> getNext() {
			...
			if (z > zMax) return null;
			Iterable<T> tmp = grid.getObjectsAt(x, y, z);
			y++;
			return tmp;
		}
	...
	/**
	 * Creates a Moore query on the specified grid using the specified object
	...
	/**
	 * Gets an iterable over all the objects that make up the Moore neighborhood of
	 * the source object. The source object and neighborhood extent are specified in the
	 * constructor or the reset method. The order of the objects returned is back to front
	 * by column starting with the left side column.<p>
	 *
	...
	public Iterable<T> query() {
		...
		if (size == 1) {
			...
			return new IterableAdaptor<T>(new Iterator2D(mins[0], mins[1], maxs[0], maxs[1], (int)px, (int)py));
		} else {
			double py = point.getY();
			int pz = (int)point.getZ();
			return new IterableAdaptor<T>(new Iterator3D(mins[0], mins[1], mins[2],
							maxs[0], maxs[1], maxs[2], (int) px, (int) py, pz));
		}
	}
	...
	private void setupMinMax(int size) {
		for (int i = 0; i < size; i++) {
			...
			maxs[i] = (int)max;
			if (!grid.isPeriodic()) {
				int origin = (int) dims.dimensions[i];
	 ...
	 * extent are specified in the constructor or the reset method. The order of the objects returned
	 * is back to front by column starting with the left side column.<p>
	 *
	 * This will return an empty iterable if the  source object does not have a grid
	 * location.
	 *
	...
	public Iterable<T> query(Iterable<T> iter) {
		...
		if (size == 2) {
			return new FilteredIterator<T>(new Iterator2D(mins[0], mins[1], maxs[0], maxs[1], (int)px, (int)py),
							new Contains<T>(set));
		} else {
			int pz = (int)point.getZ();
			return new FilteredIterator<T>(new Iterator3D(mins[0], mins[1], mins[2],
							maxs[0], maxs[1], maxs[2], (int) px, (int) py, pz),
							new Contains<T>(set));
		}
	}
