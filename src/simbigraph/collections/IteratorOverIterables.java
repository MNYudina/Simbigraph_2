package simbigraph.collections;
 ...
 * multiple iterables. The iterator will iterate over all the objects
 * returned by each iterable.
 *
 * @author Nick Collier
...
public abstract class IteratorOverIterables<T> implements Iterator<T> {
	...
	protected T getNextObj() {
		...
		while (obj == null) {
			if (nextIter.hasNext()) obj = nextIter.next();
			else {
				Iterable<T> iterable = getNext();
	 ...
	 */
	public T next() {
		if (nextObj == null) throw new NoSuchElementException();
		T tmp = nextObj;
		nextObj = getNextObj();
		return tmp;
	}
	 ...
	 */
	public void remove() {
		if (nextIter != null) nextIter.remove();
